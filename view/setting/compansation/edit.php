...
$sqlGet = "select * from compansation";
                ...
                <p><h2 >เลือกสินค้า</h2></p>
                    <select class="w3-input" name="data[product_id]" required id="">
                        ...
                        <?php foreach($products as $key=>$value) { ?>
                            <option value="<?php echo $value['id'] ?>" <?php if($data['product_id'] == $value['id']) { echo "selected"; } ?> ><?php echo $value['title'] ?></option>
                        <?php } ?>
                    </select>
                    <p><h2>ราคาค่าตอบแทนต่อหน่วย</h2></p>
                    <input value="<?php echo $data['price'] ?>" class="w3-input w3" required type="text" name="data[price]">
                    <br><br>
