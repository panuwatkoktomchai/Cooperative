...
$sql = "select produce.* , users.name as user_name, CONCAT(customers.firstname,' ',customers.lastname) as cus_name from produce join users on produce.user_id = users.id join customers on produce.customer_id = customers.id order by created_at desc";
$data = $conn->query($sql);
